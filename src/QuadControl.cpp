...
void QuadControl::Init()
{
  ...
  integratedAltitudeError = 0;						//积分高度误差
...
V3F QuadControl::BodyRateControl(V3F pqrCmd, V3F pqr)
{
  ...
  V3F Iv3f;
  Iv3f = Ixx, Iyy, Izz;
  momentCmd = (kpPQR * (pqrCmd - pqr))*Iv3f;
  /////////////////////////////// END STUDENT CODE ////////////////////////////
...
V3F QuadControl::RollPitchControl(V3F accelCmd, Quaternion<float> attitude, float collThrustCmd)
{
  ...
  float c_d = 0;
  c_d = -1*collThrustCmd / mass; //why negative??
  if (collThrustCmd > 0)
  {	  
	  ...
	  float b_x_command_dot = kpBank * (target_R13 - R(0, 2));
	  float b_y_command_dot = kpBank * (target_R23 - R(1, 2));
	  pqrCmd.x = 1 / R(2, 2)*(R(1, 0)*b_x_command_dot - R(0, 0)*b_y_command_dot);
	  pqrCmd.y = 1 / R(2, 2)*(R(1, 1)*b_x_command_dot - R(0, 1)*b_y_command_dot);
	  pqrCmd.z = 0;
  }
